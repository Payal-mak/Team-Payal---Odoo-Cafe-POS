...
export const createOrder = async (req, res) => {
    try {
        ...
        const orderResult = await query(
            ...
            [orderNumber, session_id, table_id, customer_name || null, totalAmount, taxAmount, notes || null]
        );
        ...
        for (const item of items) {
            ...
            await query(
                ...
                [orderId, item.id, item.quantity, item.price, item.tax_rate || 10, lineSubtotal, lineTotal]
            );
        }
...
export const sendToKitchen = async (req, res) => {
    try {
        ...
        await query(
            `UPDATE orders SET kitchen_stage = 'preparing' WHERE id = ?`,
            [id]
        );
