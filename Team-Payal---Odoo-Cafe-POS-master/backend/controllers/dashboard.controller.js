...
export const getReports = async (req, res) => {
    try {
        ...
        if (responsible_user_id) {
            sql += ' AND ps.responsible_user_id = ?';
            params.push(responsible_user_id);
        }

        // Product filter (through order lines)
        if (product_id) {
            sql += ` AND o.id IN (
        ...
        orders.forEach(order => {
            if (order.payment_status === 'paid') {
                summary.total_revenue += parseFloat(order.total_amount || 0);
                summary.total_tax += parseFloat(order.tax_amount || 0);
                summary.by_status.paid++;

                if (order.payment_method && summary.by_payment_method[order.payment_method]) {
                    summary.by_payment_method[order.payment_method].count++;
                    summary.by_payment_method[order.payment_method].revenue += parseFloat(order.total_amount || 0);
                }
            } else {
                summary.by_status.unpaid++;
            }
        });
